...
from sidekick_usages.core.selection.operation import (
    AuthorityReadyProof,
    FinalizedSelection,
...
@dataclass(frozen=True, slots=True, kw_only=True)
class RelatedRuntimeAuthority:
    """Provider-owned proof relating native runtime to one saved account."""
    ...
    def __post_init__(self) -> None:
        """Normalize the exact provider observation timestamp."""
        object.__setattr__(self, "observed_at", as_utc(self.observed_at))
...
class NativeReconciliationResult:
    ...
    def __post_init__(self) -> None:
        ...
        if related is not None and (
            ...
            or self.selected.provider_id is not related.provider_id
            or self.selected.account_id != related.account_id
            or self.selected.runtime_generation != related.generation
            or self.selected.verified_at != related.observed_at
        ):
            raise ValueError("Related runtime authority lacks provider proof.")
