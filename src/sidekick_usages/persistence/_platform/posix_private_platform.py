...
class PosixPrivateCredentialPlatform:
    ...
    def repair_permissions(self, root: Path) -> tuple[int, int]:
        ...
        with posix_private._open_repair_tree(root) as opened:
            if opened is None:
                return (0, 0)
            directories, identities = posix_private._scan_repair_tree(opened)
            repaired = sum(
                posix_private._repair_directory_permissions(
                    opened,
                    directory,
                    identities,
                )
            ...
            posix_private._scan_tree(opened)
            return (repaired, 0)
    ...
    def contains_artifacts(self, root: Path) -> bool:
        ...
        self._qualifier.qualify(root)
        with posix_private._open_tree(root) as opened:
            if opened is None:
                return False
            entries, _identities = posix_private._scan_tree(opened)
            return bool(entries)
    ...
    def list_directories_shallow(self, root: Path) -> tuple[str, ...]:
        ...
        with posix_private._open_tree(root) as opened:
            ...
            return tuple(
                sorted(
                    entry.relative[0]
                    for entry in posix_private._scan_direct_tree(opened)
                    if entry.directory
                )
            )
    ...
    def destroy_artifacts(self, root: Path) -> None:
        ...
        with posix_private._open_tree(root) as opened:
            if opened is None:
                return
            entries, identities = posix_private._scan_tree(opened)
            for entry in sorted(
                entries,
                key=lambda candidate: (
                    len(candidate.relative),
                ...
                posix_private._delete_entry(opened, entry, identities)
            remaining, _remaining_identities = posix_private._scan_tree(opened)
            if remaining:
                raise posix_private._native_error(NativeFailureKind.CHANGED)
    ...
    def destroy_tree(self, root: Path) -> None:
        ...
        with posix_private._open_tree(root) as opened:
            ...
            if remaining:
                raise posix_private._native_error(NativeFailureKind.CHANGED)
            posix_private._require_root_identity(opened)
            root_entry = posix_private._TreeEntry(
                (opened.root_basename,),
                opened.root_identity,
                True,
            )
            posix_private._delete_directory(
                opened,
