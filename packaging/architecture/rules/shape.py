...
_APPROVED_FLAT_NAMESPACE_FAMILIES = {
    (
        PurePosixPath("src/sidekick_usages/persistence/schema"),
        "prefix",
        "selection",
    ): frozenset({"selection.py", "selection_operation.py"}),
}
...
def _check_flat_namespaces(
    ...
    for (parent, kind, token), members in sorted(
        ...
        approved = _APPROVED_FLAT_NAMESPACE_FAMILIES.get((parent, kind, token))
        if approved == frozenset(unit.path.name for unit in members):
            continue
